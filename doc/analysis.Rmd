...
```{r, GYTPlot, echo = FALSE, fig.fullwidth = TRUE, fig.cap="GYT biplot to show the combined relationships of genotypes performance of yield with seed protein and lodging.", fig.width=6, fig.height=6, message=FALSE, warning=FALSE}
readd(GYTModel) %>% plot(size.text.gen = 3)
```
<break>

Genotype vectors with small angles to the Yield\*Trait of interest have higher values of such a Yielld\*Trait combination. This means that to find genotypes with high values of both yield and protein, we should look for genotype lines which fall close to the green Y*Protein line. In this case, N18-1796, N18-1575, and N18-1751 appear to be the closest. Going back to the BLUP table, we can see that both these genotypes do indeed have both high protein, and yield comparable to the Dunphy check cultivar. Let's see where the BLUPs for these genotypes fall on the distributions for the three traits of interest. 

```{r, BLUP_histogram_Plot_selections, echo = FALSE, fig.fullwidth = TRUE, fig.cap="Histograms of BLUPs for seed yield, seed oil, and seed protein. The BLUPs for parents and checks are labeled for each trait.", fig.width=12, fig.height=6, message=FALSE, warning=FALSE}
